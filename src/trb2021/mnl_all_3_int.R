...
apollo_control <- list(modelName = "mnl_all_3_int",
...
vars = c(
  ...
  "Edc_MoreBachelor",
  "Gen_NonFemale",
  "Gen_NonFemale_Inc_Mid",
  "Gen_NonFemale_Inc_High",
  "Chl_Yes",
  "Ngh_NonUrban",
  "Rac_Other"
)
  ...
  function(apollo_beta,
    ...
    for (i in alts) {
      ...
      V[[i]] = 
        ...
        get(varList[14]) * (Gen == 2) * (Inc == 2) +
        get(varList[15]) * (Gen == 2) * (Inc == 3) +
        get(varList[16]) * (Chl == 2) +
        get(varList[17]) * (Ngh == 2) +
        get(varList[18]) * (Rac == 2)
    }
