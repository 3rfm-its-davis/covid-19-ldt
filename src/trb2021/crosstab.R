socioCross <- raw %>%
  selectColumns(c("ResponseId", "City", "B02_5", "D01", "D09_2", "C07", "C12_3", "C12_13", "C13_3", "I01_1", "I04_2", "I04_3", "I04_1", "I04_5", "I04_4", "I05", "I09_USD", "I07", "I12")) %>%
...
count <- raw %>%
  ...
  mutate(
    t19 = C11.1_1_1 + C11.1_1_2 + C11.1_1_3,
    t20 = C11.2_1_1 + C11.2_1_2 + C11.2_1_3
  ) %>%
  select(ResponseId, t19, t20)

inner_join(joined, count) %>%
  group_by(biz_all) %>%
  summarize(m19 = mean(t19), s19 = sd(t19), m20 = mean(t20), s20 = sd(t20))
inner_join(joined, count) %>%
  mutate(biz_all = case_when(biz_all == "zero" ~ "zero", T ~ "non-zero")) %>%
  group_by(biz_all) %>%
  summarize(m19 = mean(t19), s19 = sd(t19), m20 = mean(t20), s20 = sd(t20))
inner_join(joined, count) %>%
  mutate(biz_all = case_when(T ~ "all")) %>%
  group_by(biz_all) %>%
  summarize(m19 = mean(t19), s19 = sd(t19), m20 = mean(t20), s20 = sd(t20))
inner_join(joined, socioCross) %>%
  inner_join(count) %>%
  group_by(Fut) %>%
  summarize(m19 = mean(t19), s19 = sd(t19), m20 = mean(t20), s20 = sd(t20))
